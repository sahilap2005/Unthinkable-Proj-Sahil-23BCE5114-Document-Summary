...
import { File, FileText, FileType, Image as ImageIcon, X } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-6 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 flex items-center justify-between shadow-sm">
      <div className="flex items-center space-x-4">
        <div className={`p-3 rounded-lg ${isPdf ? 'bg-red-50 text-red-500 dark:bg-red-900/20' : isDocx ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30' : 'bg-emerald-50 text-emerald-500 dark:bg-emerald-900/20'}`}>
          {isPdf ? <FileText size={24} /> : isDocx ? <FileType size={24} /> : <ImageIcon size={24} />}
        </div>
        ...
        <div className="flex flex-col">
          ...
          <span className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
            {isPdf ? 'PDF Document' : isDocx ? 'Word Document' : 'Image File'} • {sizeMB} MB
          </span>
        </div>
      </div>
